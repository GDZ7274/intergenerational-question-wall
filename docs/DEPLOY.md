...
当前公开页缓存版本为 `styles.css?v=16`、`app.js?v=16`、`backend.js?v=4`、`config.js?v=3`，后台为 `admin.css?v=5`、`admin.js?v=6`。Lucide 1.8.0 从 `prototype/vendor/lucide.min.js` 自托管。公开页和后台的 CSP 默认只允许同源脚本与样式，网络连接仅放行同源和 `https://*.supabase.co`。更新这些文件时应同步递增对应查询参数，避免手机端继续命中旧缓存。
