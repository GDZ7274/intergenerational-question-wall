...
| W01 | `wall` 单张问答墙 | 所有人 | 上下滑切换便签、轻点查看详情 |
| W02 | 便签详情弹层 | 所有人 | 查看同一问题的全部公开回答、收藏或举报 |
| A01 | `identity` 身份选择 | 未选择身份的参与者 | 选择大人或小朋友，并继续刚才的动作 |
| A02 | `participate` 参与页 | 已选择身份的参与者 | 选择提问或回答 |
| Q01 | `ask` 提问编辑 | 已选择身份的参与者 | 输入问题、选择匿名方式、预览并提交 |
| R01 | `pool` 待回答问题池 | 已选择身份的参与者 | 浏览当前身份可回答的问题 |
| R02 | `answer` 回答编辑 | 已选择身份的参与者 | 输入回答、预览并提交 |
| M01 | `mine` 我的 | 所有人 | 查看本机记录、同步状态、导入恢复码与驳回重投 |
...
移动端以滑动为主：问答墙固定视口一次只显示一张便签，整张卡片可轻点；问题池纵向滚动，整张问题卡进入回答。底部导航只保留“墙 / 参与 / 我的”，浏览页不再堆叠“下一张”“再看一张”等重复主按钮。
